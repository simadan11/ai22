# hub.py — EDIT GEOINT / OSINT Hub: Interactive Military Sites & Satellite Intelligence
"""
...
marking active, abandoned, and historical military bases, radar sites, airfields, bunkers, naval stations, and equipment locations.
...
class OSINTHub(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("🛰️ EDIT GEOINT / OSINT Hub — Карта военных баз, техника и спутник (MAX GEOINT)")
        ...
        right_split.setSizes([620, 300])
        main_layout.addWidget(right_split, 1)

        self.statusBar().showMessage("🛰️ GEOINT Hub готов • База военных и исторических объектов загружена • OSINT Mode активен")
    ...
    def _build_no_webengine_view(self) -> QWidget:
        ...
        lay = QVBoxLayout(w)
        lbl = QLabel(
            "🛰️ Интерактивная карта GEOINT (Leaflet) готова!\n\n"
            "PyQt6-WebEngine не установлен в текущем окружении.\n"
            "Нажмите кнопку ниже, чтобы открыть полноценную карту с 40+ базами в вашем браузере:"
        )
        lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lbl.setStyleSheet("color: #8ffcff; font-size: 13px; font-weight: bold;")
        lay.addWidget(lbl)

        open_btn = QPushButton("🌐 ОТКРЫТЬ GEOINT КАРТУ В БРАУЗЕРЕ (Google Maps / Спутник / OSM)")
        ...
        """)
        open_btn.clicked.connect(lambda: open_map_in_browser(self.current_site))
    ...
    def _on_list_item_clicked(self, item: QListWidgetItem):
        idx = self.site_list_widget.row(item)
        if 0 <= idx < len(MILITARY_SITES):
            self.select_site_by_index(idx)
    ...
    def _build_left_panel(self):
        ...
        btn_his = QPushButton("🏛️ Исторические")
        btn_his.clicked.connect(lambda: self.filter_map_category("historic"))
        cat_box2.addWidget(btn_air)
        cat_box2.addWidget(btn_bnk)
        cat_box2.addWidget(btn_his)
        ...
        ai_btn = QPushButton("🧠 AI GEOINT АНАЛИЗ ОБЪЕКТА")
    ...
    def _build_satellite_panel(self):
        ...
        header = QLabel("🛰️ СПУТНИКОВАЯ РАЗВЕДКА • ОТЧЁТ И ССЫЛКИ")
    ...
    def display_site_report(self, site: dict):
        ...
        report += f" • Google Maps     : {links['google_maps']}\n"
        report += f" • OpenStreetMap   : {links['openstreetmap']}\n"
        report += f" • WikiMapia       : {links['wikimapia']}\n"

        self.result_text.setPlainText(report)
        self.sat_report_view.setPlainText(
            f"🛰️ GEOINT SUMMARY: {site['name'].upper()}\n"
            f"{'='*50}\n"
            f"Координаты GPS  : {site['lat']:.4f}° N, {site['lon']:.4f}° E\n"
            f"Статус объекта  : {site['status']}\n"
            f"Тип сооружения  : {site['type']}\n"
            f"Спутник Google  : {links['google_satellite']}\n\n"
            f"ОПИСАНИЕ:\n{site['description']}\n\n"
            f"ОСНАЩЕНИЕ:\n{site['equipment']}"
        )
    ...
    def _on_combo_changed(self, index: int):
        idx = self.site_combo.itemData(index)
        if idx is not None and isinstance(idx, int) and 0 <= idx < len(MILITARY_SITES):
            self.select_site_by_index(idx)

    def select_site_by_index(self, idx: int):
        site = MILITARY_SITES[idx]
        self.display_site_report(site)
        self.html_map_path = generate_html_map(target_site=site)
        if WEBENGINE_AVAILABLE and hasattr(self, "map_view"):
            self.map_view.setUrl(QUrl.fromLocalFile(str(self.html_map_path)))

    def filter_map_category(self, cat: str):
        self.html_map_path = generate_html_map(target_site=self.current_site, filter_category=cat)
        if WEBENGINE_AVAILABLE and hasattr(self, "map_view"):
            self.map_view.setUrl(QUrl.fromLocalFile(str(self.html_map_path)))
        self.result_text.append(f"\n🗂️ Карта отфильтрована по категории: {cat.upper()}")
        self.statusBar().showMessage(f"🛰️ Фильтр объектов: {cat.upper()}")

    def _change_basemap(self, index: int):
        if not WEBENGINE_AVAILABLE or not hasattr(self, "map_view"):
            return
        platform = self.platform_combo.currentText()
        # In Leaflet we let users toggle layers via upper right control, but we can append reminder
        self.statusBar().showMessage(f"🛰️ Выбран слой: {platform}")

    def _open_in_external_browser(self):
        msg = open_map_in_browser(self.current_site)
        self.result_text.append(f"\n🌐 {msg}")
    ...
    def search_location(self):
        ...
        if matches:
            ...
            # Если не нашлось в нашей базе — попробуем проверить парсинг координат lat, lon
            try:
                ...
                if len(parts) == 2:
                    ...
                    self.html_map_path = generate_html_map(target_site=custom_site)
                    if WEBENGINE_AVAILABLE and hasattr(self, "map_view"):
                        self.map_view.setUrl(QUrl.fromLocalFile(str(self.html_map_path)))
                    return
            except Exception:
    ...
    def enhance_with_ai(self):
        ...
        ai_report = (
            f"🧠 AI GEOINT ASSESSMENT REPORT\n"
            f"{'='*50}\n"
            f"ОБЪЕКТ   : {site['name']} ({site['country']})\n"
            f"КАТЕГОРИЯ: {site['type']} [{site['status']}]\n"
            f"КООРДИНАТЫ: {site['lat']}, {site['lon']}\n"
            f"{'='*50}\n"
            f"1. СТРАТЕГИЧЕСКАЯ ОЦЕНКА:\n"
            f"   Объект имеет высокую значимость в рамках геопространственной разведки.\n"
            f"   Описание: {site['description']}\n\n"
            f"2. ИНФРАСТРУКТУРА И ОСНАЩЕНИЕ:\n"
            f"   Зафиксированные элементы: {site['equipment']}.\n\n"
            f"3. РЕКОМЕНДОВАННЫЕ ДЕЙСТВИЯ (OSINT/GEOINT):\n"
            f"   • Переключить слой на 'Google Satellite' или 'Esri Imagery' для детального визуального осмотра.\n"
            f"   • Использовать WikiMapia для изучения исторической экспликации сооружений.\n"
            f"   • Проверить периметр и подъездные пути через OpenStreetMap.\n"
        )
        self.sat_report_view.setPlainText(ai_report)
